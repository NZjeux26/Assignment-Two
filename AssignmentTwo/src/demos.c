    ...
    while(play){
        ...
        for(int i=0; i <BLOCKC; i++){
            ...
            BLOCKC = 2;//resets the blocks to only be two at the start, or else it will start with the amount of blocks you finished your last game with
                ...
                float ldist = ((blocks[i].x-player.x)*(blocks[i].x-player.x)+(blocks[i].y-player.y)*(blocks[i].y-player.y));
                float rdist = (blocks[i].x-(player.x+player.w))*(blocks[i].x-(player.x+player.w))+(blocks[i].y-player.y)*(blocks[i].y-player.y);

                if(ldist<(blocks[i].w*blocks[i].w/4) || rdist<(blocks[i].w*blocks[i].w*blocks[i].w/4)){
                    float nx;
                    if(ldist<(blocks[i].w*
                    ))
                }
